...
export const Button = ({classes, disabled, callToAction, textButton}: ButtonProps) => {
  return (
    <button 
      ...
      className={`${classes ? classes : ''} ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:from-yellow-300 hover:via-yellow-700 hover:to-yellow-300 hover:scale-105'} w-3xs text-white font-medium text-md md:text-2xl bg-gradient-to-r from-yellow-200 via-yellow-600 to-yellow-200 py-3 px-8 rounded-full shadow-lg transition-all duration-300 transform -mt-5`}
    >
      {textButton}
    </button>
  )
